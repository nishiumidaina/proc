...
$pdo = new PDO(
    ...
    $sql = 'INSERT INTO カメラ (カメラ設置場所, 緯度, 経度, 動画埋め込み) VALUES (:movie_name, :movie_lat, :movie_long, :movie)';
    // DBに保存
    $stmt = $pdo->prepare($sql);
    $stmt->bindValue(':movie_name', $_POST['movie_name']);
    $stmt->bindValue(':movie_lat', $_POST['movie_lat']);
    $stmt->bindValue(':movie_long', $_POST['movie_long']);
    $stmt->bindValue(':movie', $_POST['movie']);
    $stmt->execute();
?>
<!DOCTYPE html>
<html lang="en">
...
<a href="./camera_input.php">カメラ登録フォームへ</a>
</div>
</body>
</html>
