...
try {
    ...
    $stmt = $dbh->query('SELECT * FROM 登録情報');
    $result = 0;
...
<html>
  ...
  <body>
    ...
    開発プロジェクト
      ...
      <div class="zentai">
      <h2>登録済み観光地情報一覧</h2>
      <h5>こちらは登録用データベースに保管された観光地です。</h5>
        <?php
            echo "<table>\n";
              echo "<tr>\n";
              echo "<th>観光地名</th><th>緯度</th><th>経度</th><th>住所</th><th>電話番号</th><th>観光地説明</th><th>画像</th><th>パズル画像</th>\n";
              echo "</tr>\n";
              foreach ($result as $user) {
                echo "<tr>\n";
                echo "<td>" . $user["登録観光地名"] . "</td>\n";
                echo "<td>" . $user["登録緯度"] . "</td>\n";
                echo "<td>" . $user["登録経度"] . "</td>\n";
                echo "<td>" . $user["登録住所"] . "</td>\n";
                echo "<td>" . $user["登録電話番号"] . "</td>\n";
                echo "<td>" . $user["登録観光地説明"] . "</td>\n";
                echo '<td><img src="../img/' , $user['登録画像'] , '" widht=50 height=50></td>';
                echo "<td>\n";
                echo "<a href=delete_new.php?id=". $user["登録ID"] . ">削除</a>\n";
                echo "</td>\n";
                echo "</td>\n";
                echo "</tr>\n";
              }
